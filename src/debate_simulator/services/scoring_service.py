...
class ScoringService:
    ...
    def compute_score(self, breakdown: dict[str, float], penalties: list[Penalty]) -> Score:
        ...
        weighted_total = sum(breakdown.get(dimension, 0.0) * weight for dimension, weight in self.weights.items())
        penalty_total = sum(penalty.points for penalty in penalties)
        return Score(total=max(weighted_total + penalty_total, 0.0), breakdown=breakdown, penalties_applied=penalties)
